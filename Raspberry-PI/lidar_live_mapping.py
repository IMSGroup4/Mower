...
def lidar_log():
    ...
    print(health)
    text_file = open("lidar_mapping.txt", "a")
    text_file.truncate(0) #this wipes the data in the txt file
    for i, scan in enumerate(lidar.iter_measures()):
        if scan[3] != 0:
            text_file.write(str(scan))
            text_file.write('\n')
        if i == 10000: #iterate of 10000 times i. should give around 3000 values
            break
def loadData():
    data_file = open("lidar_mapping.txt")
    lidar_data = []
